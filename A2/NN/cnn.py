...
def Conv2DBackward(grad_y, x, y, w):
    ...
    for c in xrange(C):
        for k in xrange(K):
            for i in xrange(I):
                for j in xrange(J):
                    wt[i, j, k, c] = wtmp[I-i-1, J-j-1, c, k]
    grad_x = Conv2D(grad_y, wt, pad=[I-1,J-1])
    x = np.transpose(x, [3, 1, 2, 0])
    grad_y_tmp = np.transpose(grad_y, [1, 2, 0, 3])
    grad_w = Conv2D(x, grad_y_tmp, pad=[I-1,J-1])
...
def main():
    ...
    for i in xrange(model['W1'].shape[3]):
        ...
        plt.imshow(model['W1'][:, :, 0, i].reshape(5, 5),cmap=plt.cm.gray)
    plt.draw()
    raw_input('Press Enter.')
    # Uncomment if you wish to save the model.
    model_fname = "cnn_f0=48_f1=48.model"
