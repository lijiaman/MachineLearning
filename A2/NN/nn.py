...
def ReLUBackward(grad_y, x, y):
    ...
    ###########################
    tmp = x.copy()
    tmp[tmp>0] = 1#not >= !!Made some mistakes before..
    tmp[tmp<=0] = 0
    grad_x = grad_y * tmp
    return grad_x
    raise Exception('Not implemented')
...
def main():
    ...
    num_hiddens = [16, 32]#default [16, 32]
    eps = 0.01
    momentum = 0.0
    num_epochs = 1000
    batch_size = 100
    ...
    plt.figure(0)
    plt.clf()
    for i in xrange(model['W1'].shape[1]):
        plt.subplot(1, model['W1'].shape[1], i+1)
        plt.imshow(model['W1'][:, i].reshape(48, 48), cmap=plt.cm.gray)
    plt.draw()
    raw_input('Press Enter.')
    # Uncomment if you wish to save the model.
    model_fname = 'nn_model_i1000_100_100.npz'
